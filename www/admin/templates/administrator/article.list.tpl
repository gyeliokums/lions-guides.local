...
{block name="body_main_content_page_heading"}
	...
	<header class="panel-heading custom-tab ">
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="?model=article&amp;parent_id={$data.common.parent_id}">Статьи</a></li>
...
{block name="body_main_content_page_main"}
	<div class="panel-body">
		<a href="?model=article&amp;action=add&amp;parent_id={$data.common.parent_id}">Добавить статью</a>
		<table class="table">
			<thead>
			<tr>
				<td>URL</td>
			...
			{foreach $data.article.data as $article}
				<tr>
					<td>{$article.inner_name}</td>
					<td>{$article.name}</td>
					<td><a href="?model=article&amp;action=update&amp;id={$article.id}">Изменить</a></td>
					<td><a href="?model=article&amp;id={$article.id}&amp;parent_id={$article.parent_id}&amp;action=remove&amp;submit=save|close">Удалить</a></td>
				</tr>
			{/foreach}
		</table>
		<a href="?model=article&amp;action=add&amp;parent_id={$data.common.parent_id}">Добавить статью</a>
