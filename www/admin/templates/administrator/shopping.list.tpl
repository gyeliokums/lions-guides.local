...
{block name="body_main_content_page_heading"}
	...
	<header class="panel-heading custom-tab ">
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="?model=shopping&amp;parent_id={$data.common.parent_id}">Шоппинг</a></li>
...
{block name="body_main_content_page_main"}
	<div class="panel-body">
		<a href="?model=shopping&amp;action=add&amp;parent_id={$data.common.parent_id}">Добавить статью</a>
		<table class="table">
			<thead>
			<tr>
				<td>URL</td>
			...
			{foreach $data.shopping.data as $shopping}
				<tr>
					<td>{$shopping.inner_name}</td>
					<td>{$shopping.name}</td>
					<td><a href="?model=shopping&amp;action=update&amp;id={$shopping.id}">Изменить</a></td>
					<td><a href="?model=shopping&amp;id={$shopping.id}&amp;parent_id={$shopping.parent_id}&amp;action=remove&amp;submit=save|close">Удалить</a></td>
				</tr>
			{/foreach}
		</table>
		<a href="?model=shopping&amp;action=add&amp;parent_id={$data.common.parent_id}">Добавить статью</a>
