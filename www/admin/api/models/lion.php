...
namespace API\Models {
	class Lion extends AbstractModel {
		static protected
			$_basicTableName = 'lion',
			$_innerKeys = array(
				'parent_id',
				'inner_name',
				'title',
		...
		protected function __construct() {
			...
			$this->_assistants[] = new \API\Assistants\LionImagePreview();
			$this->_serializedFields = array('coordinates');
		}
		...
		protected function _createPathToUploads() {
			$data = \API\Data\Collector::instance();
			$ptui = '../uploads/images/stateunion/' . $data->get('country', 'position') . '/country/' . $data->get('city', 'position') . '/city/' . $data->get('lion', 'position') . '/lion/' . $data->get('id') . '/';
